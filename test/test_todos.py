...
from ..router.todo import get_db, get_current_user
from .utils import *
...
def test_read_all_authenticated(test_todo):
    ...
    print(response.json())
    assert response.json() == [{'complete': False, 'title': 'Learn to code', 'description': 'Need to learn everyday', 'id': 1, 'priority': '5',
      'owner_id': 1}]
...
def test_read_not_authenticated(test_todo):
...
def test_not_update_todo(test_todo):
    request_data = {
        'title': 'Change the title of the todo already saved',
        'description': 'Need to learn everyday',
        'priority': 5,
...
def test_delete_not_todo(test_todo):
